...
export class App extends React.Component {
    ...
    _onSearch(event) {
        ...
        this.setState({
            redirectPath: this.state.redirectUri
        })
    }
    _onSearchInput(event) {
        let pathname = uriToPath(event.target.value);
        this.setState({
            ...
            <div style={{width: (window.innerWidth - 20) + 'px',height: (window.innerHeight - 160) + 'px'}}>
                ...
                <header>
    				...
    				<form onSubmit={this._onSearch}>
    					<input type="search" id="search" onChange={this._onSearchInput} placeholder="Search" />
    				</form> 
    			</header>
    			...
    			<main>
    				<aside style={{width: '200pt'}}>
    					...
    					<div id="nowplaying" >
    						...
    						<div id="nowplaying_image" style={{backgroundSize: 'cover', backgroundImage: 'url("' + (this.state.item && this.state.item.album.images.length > 0 ? this.state.item.album.images[0].url: '') + '")', flex: 1}}></div>
    					</div>
    				</aside>
    				<div id="viewstack">
