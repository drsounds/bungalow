...
Music.prototype.login = function () {
  ...
  var t = setInterval(function () {
  	if (!loginWindow) {
		...
		var uris = ['bungalow:user:drsounds:playlist:782AlbjY7wiTvocWhN5xuk', 'bungalow:year:2009', 'bungalow:user:rebeccasandberg90', 'bungalow:doctrine:bungalow'];
		for (var i = 0; i < uris.length; i++) {
			var li = document.createElement('li');
			li.setAttribute('data-load-uri', uris[i]);
	...
	music.login().then(function (ready) {
		...
		music.getUserPlaylists(function (playlists) {
			for (var i = 0; i < playlists.length && i < 100; i++) {
				...
				listItem.innerHTML = '<li data-uri="' + playlist.uri + '"><i class="fa fa-music"></i> ' + playlist.name;
				/*+ ' <span class="fade">by ' + playlist.user.displayName + '</span></li>'*/listItem.setAttribute('data-uri', playlist.uri);
...
},{"@bungalow/models":3,"@bungalow/views":5,"jquery":6}],2:[function(require,module,exports){
"use strict";
...
},{"@bungalow/cosmos":2}],4:[function(require,module,exports){
/*!
 ...
 * @class
 */
var Track = function Track(track, index, contextUri, options) {
    if ('track' in track) {
 ...
 */
var Image = function Image(object, options) {
 ...
 * @param {Object} options [description]
 */
var Player = function Player(object, options) {
    this.node = document.createElement('div');
};
...
var Collection = function Collection(collection, options, Class) {
...
var PopularityBar = function PopularityBar(resource) {
...
_exports.PopularityBar = PopularityBar;
 ...
 * @class
 */
var TrackContext = function TrackContext(resource, options) {
...
_exports.TrackContext = TrackContext;
...
_exports.showThrobber = showThrobber;
_exports.hideThrobber = hideThrobber;

var CollectionItem = function CollectionItem(object, options) {
    this.node = document.createElement('div');
 ...
 * @constructor
 */
var Card = function Card(resource, options, type) {
    this.node = document.createElement('div');
...
var AlbumCardCollection = function AlbumCardCollection(resource, options, coverSize) {
...
var CardCollection = function CardCollection(collection, options, type) {
...
_exports.Card = Card;
_exports.AlbumCardCollection = AlbumCardCollection;
_exports.CardCollection = CardCollection;

var AlbumCollection = function AlbumCollection(resource, options, coverSize, type) {
...
var Feed = function Feed(resource, options, coverSize) {
...
_exports.Feed = Feed;
var views = _exports;
var Post = function Post(post, options) {
 ...
 * @constructor
 */
var Album = function Album(album, options) {
    var self = this;
    this.node = document.createElement('tbody');
    ...
    var tr = document.createElement('tr');
    if (album instanceof models.Album) {} else {
 ...
 * @constructor
 */
var Playlist = function Playlist(playlist, options) {
    var self = this;
    this.node = document.createElement('tr');

    if (playlist instanceof models.Playlist) {} else {
...
_exports.AlbumCollection = AlbumCollection;
 ...
 **/
var Bungalow = function Bungalow(section, options) {
...
var SimpleHeader = function SimpleHeader(resource, options) {
    this.node = document.createElement('table');
    ...
    console.log(resource);
    td2.innerHTML = '<small class="sp-type">' + resource.type.toUpperCase() + '</small>' + '<h2><a data-uri="' + resource.uri + '">' + resource.name + '</a>';
    if ('owner' in resource) td2.innerHTML += 'by <a data-uri="' + resource.owner.uri + '">' + resource.owner.id + '</a></h2>';
    ...
    if ('description' in resource) td2.innerHTML += '<p class="description">' + resource.description.bungalowize();+'</p>';
};

_exports.SimpleHeader = SimpleHeader;

var CoverImage = function CoverImage(resource, size) {
    this.node = document.createElement('div');
    ...
    if ('images' in resource && resource.images.length > 0) image.style.backgroundImage = 'url("' + resource.images[0].url + '")';
    image.style.width = size + 'px';
    image.style.height = size + 'px';
    this.node.appendChild(image);
};

var Header = function Header(resource, options, coverSize) {
...
var TabBar = function TabBar(data) {
    this.node = document.createElement('div');
...
var TabBarTab = function TabBarTab(data) {
    Object.assign(this, data);
...
_exports.TabBar = TabBar;
_exports.TabBarTab = TabBarTab;
...
},{"@bungalow/models":3,"jquery":4}],6:[function(require,module,exports){
