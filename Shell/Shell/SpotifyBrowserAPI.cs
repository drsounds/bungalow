/// Source from Soundbounce project
using System;
...
namespace SoundBounce.WindowsClient
{
    ...
    /// 
    /// </summary>
    public class SpotifyBrowserAPI
    {
        ...
        {
            return "4.1.1"; // ConfigurationManager.AppSettings["ClientVersion"];
        }

        public void login(string username, string password)
        {
            this.username = username;
            Task task = Task.Factory.StartNew(() => SpotifyAPI.Spotify.Login(username, password));
        }
        ...
        {
            SpotifyAPI.Spotify.PostMessage(StarTrack, new object[] { trackId });
        }
        ...
        public delegate void SpotifyMessageEventHandler(object sender, string message);
        public event SpotifyMessageEventHandler MessageReceived;
        ...
        private void PlayTrack(object[] args)
        {
            ...
            if (error != libspotify.sp_error.OK)
            {
                ...
                {
                    MessageReceived(this, error.ToString());
                }
        ...
        private void StarTrack(object[] args)
        {
            ...
            try
            {
                ...
                lock (Session._lock)
                {
                    ...
                    if (error != libspotify.sp_error.OK)
                    {
                        MessageReceived(this, "Unable to star: " + error.ToString());
                        return;
                    }
                }
            }
